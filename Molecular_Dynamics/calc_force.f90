subroutine calc_force
    ...
    do jj=1, (n_part - 1)
        ...
        do kk= jj+1, n_part !positions of particle other than (jj)th one 
            ...
            !as force and potential is zero beyond r_c in our model
            if ((r_12.le.r_c)) then
                ! write(*,*) r_12
                ...
                force(3*kk -2) = force(3*kk -2) - force_lj*(dx/(r_12)) !along x
                force(3*kk -1) = force(3*kk -1) - force_lj*(dy/(r_12)) !along y
                force(3*kk)    = force(3*kk)    - force_lj*(dz/(r_12)) !along z
