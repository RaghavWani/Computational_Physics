!Diffusion equation only
!dA/dt= D_a*d^2(A)
!dB/dt= D_b*d^2(B)

program diffusion
    implicit none
    integer, parameter :: lx=60, ly=60! BOUNDARIES 
    real*8 :: old_tempA(1:lx, 1:ly), tempA(1:lx, 1:ly)
    real*8 :: old_tempB(1:lx, 1:ly), tempB(1:lx, 1:ly)
    integer :: jj, ii
    real*8 ::  prefactor, densityA, densityB, limit 
    ...
    ! old_tempA= 0.0d0
    ! old_tempB= 0.0d0
    ! old_tempA(lx/2,ly/2) = 1.0d0
    ! old_tempA(lx/4,ly/4) = 0.1d0
    ! old_tempA(3*lx/4,ly/4) = 0.1d0
    ! old_tempA(lx/4,3*ly/4) = 0.1d0
    ! old_tempA(3*lx/4,3*ly/4) = 0.1d0
    ...
    do niter =1,tot_niter!LOOP OVER ITERATIONS.
        ...
        do jj=1,ly ! UPDATE STEP: going across a row
            ...
            do ii=1, lx !going down a column
                ...
                !solving dA/dt= D_a*d^2(A) using finite difference method
        ...
        ! CHECKING FOR CONVERGENCE AT EACH LATTICE SITE 
        ! do jj=2,ly-1
        !     do ii=2, lx-1
        ...
        ! if((niter.ge.1) .and. (niter.le.15000) )then  
        !     tempA(lx/4,3*ly/4) = 0.50d0
        ! end if
        
        ! if((niter.ge.10000) .and. (niter.le.25000) )then  
        !     tempA(lx/2,ly/2) = 1.0d0
        ! end if

        ! if((niter.ge.20000) .and. (niter.le.35000) )then  
        !     tempA(3*lx/4,ly/4) = 0.50d0
        ! end if
